...
This constitution governs AI systems that inspect, author, validate, or operate on Egolint. Applicable law and platform safety requirements, organization policy, repository policy, accepted architecture, and explicit task authority take precedence over local prompts or model defaults.
...
AI may assist Egolint's systems—Profile catalog, MegaLinter adapter, Tool configuration, Applicability resolver, Report normalizer, CLI contract, Test fixtures—but capability does not grant permission to operate them consequentially.
