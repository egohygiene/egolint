...
runtime selection, native portability/repository-contract evaluation, safe argv
construction, defensive adapter normalization, suppression evaluation,
plan/schema/report contracts, and report bookkeeping. The lightweight
`egolint` image contains only this binary and its runtime libraries.
...
The Rust domain owns versioned profile, finding, suppression, evidence,
per-tool result, plan, and run-report contracts. Adapter output is untrusted
input: normalizers whitelist supported fields into those contracts and never
copy cached process environments or arbitrary raw logs into publishable JSON.

Reports distinguish adapter-exit-only, partial, and fully normalized coverage.
This allows additional adapters to arrive incrementally without treating an
empty normalized array as proof that a tool or repository had no findings. See
[machine-readable contracts](contracts.md) for the exact semantics.

Fixes use a separate capability path. `fix` materializes two trees from the
immutable base commit, runs explicitly selected formatters against only one,
destroys its mutable Git metadata and all reports, and creates a bounded patch
in the untouched comparison repository. `apply-fix` accepts only that reviewed
patch plus its SHA-256, base commit, and expected post-tree; it never starts the
lint engine.
